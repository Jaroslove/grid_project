import {
  Component, ElementRef, AfterViewInit, OnDestroy, Output, EventEmitter,
...
export class GridCanvasComponent implements AfterViewInit, OnDestroy {
  ...
  private tx0=0; private ty0=0; private sx0=0; private sy0=0;
  private ltx=0; private lty=0; private ltt=0;
  private vx=0; private vy=0; private iid=0;
  private consumed = false; // prevent click after mousedown action
  ...
  private setupEvents(): void {
    ...
    // ── mousedown: scrollbar drag, col resize ──
    cv.addEventListener('mousedown', (e: MouseEvent) => {
      ...
      if (h.type==='h_scrollbar') {
        e.preventDefault(); this.consumed=true;
        this.svc.startHDrag(x);
        if (!this.svc.isDraggingScrollbar()) { this.svc.clickHTrack(x); this.scheduleRender(); }
        return;
      }
      if (h.type==='v_scrollbar') {
        e.preventDefault(); this.consumed=true;
        this.svc.startVDrag(y);
        if (!this.svc.isDraggingScrollbar()) { this.svc.clickVTrack(y); this.scheduleRender(); }
        return;
      }
      if (h.type==='col_resize') {
        e.preventDefault(); this.consumed=true;
        this.svc.startColResize(h.col!, e.clientX);
        return;
      }
      // row/col bracket clicks handled in click event
    });

    // ── click: brackets + cells ──
    cv.addEventListener('click', (e: MouseEvent) => {
      if (this.consumed || this.svc.isResizing() || this.svc.isDraggingScrollbar()) return;
      ...
      switch (h.type) {
        case 'row_bracket':
          if (h.key) {
            console.log('Toggle row group:', h.key);
            this.svc.toggleRowCollapse(h.key);
            this.scheduleRender();
          }
          break;
        ...
        case 'col_bracket':
          if (h.key) {
            console.log('Toggle col group:', h.key);
            this.svc.toggleColCollapse(h.key);
            this.scheduleRender();
          }
          break;
        ...
        case 'cell':
          if (h.row !== undefined && h.col !== undefined) {
            ...
            this.svc.select(h.row, h.col);
            const text = this.svc.getCellText(h.row, h.col);
            console.log(`Cell clicked: row=${h.row}, col=${h.col}, text="${text}"`);
            this.zone.run(() => {
              this.cellClick.emit({ row: h.row!, col: h.col!, text });
            });
            this.scheduleRender();
          }
          break;
    ...
    cv.addEventListener('dblclick', (e: MouseEvent) => {
      ...
      if (h?.type==='cell' && h.row!==undefined && h.col!==undefined) {
        this.showEditor(h.row, h.col);
      }
    });
    ...
    window.addEventListener('mousemove', (e: MouseEvent) => {
      ...
      if (this.svc.isResizing()) {
        this.svc.updateColResize(e.clientX);
        this.scheduleRender(); return;
      }
      const h = this.svc.hitTest(e.clientX-r.left, e.clientY-r.top);
      cv.style.cursor =
        h?.type==='col_resize' ? 'col-resize' :
        h?.type==='row_bracket'||h?.type==='col_bracket' ? 'pointer' :
        'default';
    });
    ...
    cv.addEventListener('touchstart', (e: TouchEvent) => {
      this.cancelInertia();
      const t=e.touches[0];
      this.tx0=t.clientX;this.ty0=t.clientY;this.ltx=t.clientX;this.lty=t.clientY;this.ltt=performance.now();
      this.sx0=this.svc.getScrollX();this.sy0=this.svc.getScrollY();this.vx=0;this.vy=0;
    });
    cv.addEventListener('touchmove',(e:TouchEvent)=>{
      ...
      if(this.editorVisible)this.hideEditor();this.scheduleRender();
    },{passive:false});
    cv.addEventListener('touchend',()=>{if(Math.abs(this.vx)>0.5||Math.abs(this.vy)>0.5)this.startInertia();});
  }

  private startInertia():void{this.cancelInertia();const tick=()=>{this.vx*=0.95;this.vy*=0.95;if(Math.abs(this.vx)<0.3&&Math.abs(this.vy)<0.3)return;this.svc.scrollBy(this.vx,this.vy);this.scheduleRender();this.iid=requestAnimationFrame(tick);};this.iid=requestAnimationFrame(tick);}
